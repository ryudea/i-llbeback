...
public class Exam_03 {
	public static void main (String[] args){
		...
		System.out.println(1 + x << 33); // 1 + 2 =3 , 3 << 33 = 3 * 1(	int overflow = 32) * 2^1 = 6
		System.out.println(y >= 5 || x < 0 && x > 2); // true || false = true
		System.out.println(y += 10 - x++); // y = y  + 10 , y = 15 , x++ = 2, y-x = 13 , x++ -> x = 3
		System.out.println(x+=2); // x = x + 2 = 3+ 2 = 5
		System.out.println( !('A' <= c && c <='Z') ); // c= 'A', !true = false
		System.out.println('C'-c); // 'C' = 67, 67 -65 = 2
		System.out.println('5'-'0'); // '5' - '0' = 5 - 0 = 5
		System.out.println(c+1); // 'A' + 1 = 65 + 1 = 66
		System.out.println(++c); // ++c = 'B'(일항은 int로 변환시키지 않고 char값을 유지시키면서 1 증가시킨다.)
		...
		}//numOfApples % sizeOfBucket > 0 ? (numOfApples/sizeOfBucket)+1 : numOfApple/sizeOfBucket
		}//나머지값이 0보다 크면 (사과 / 바구니)+1 0과 같거나 작으면 (사과 / 바구니 )
		
		 // (noa / sib) + (noa % sib == 0 ? 0 : 1 );
		...
		public static void main(String[] args) {
		int num = 333;
		System.out.println(  (1)  );    //( num - num%10)+1
		}
		}  
		...
		public static void main(String[] args) {
		int num = 24;
		System.out.println( (1)  ); //    ((num - num%10)+10);
		
		// ( num % 10 > 0 ?  10 - num%10 : 0) ; 
		//  (((num - num%10) + 10)-2 * (num%10)) ; = 0 //   (num / 10 + 1) * 10 - num ; 		
		...
		(int)((5f/9 * (fahrenheit - 32))*100+0.5f))/100f ;//
		...
		byte a = 10;
		byte b = 20;
		byte c = a + b;//byte c = (byte)(a+b); , int c = a + b;
		char ch = 'A'; 
		ch = ch + 2; // ch = (char)(ch + 2);
		float f = 3 / 2;// float f = (float) 3 / 2 ; , float f = 3 / 2f; 
		long l = 3000 * 3000 * 3000; //long l = (long)3000 * 3000 * 3000;
		float f2 = 0.1f;
		double d = 0.1; // 0.1f
		...
		char ch = '0'; 
		char lowerCase =(65<= ch && ch <= 88 || 97<= ch && ch <= ) ? (char)(ch+32) : ch; 
